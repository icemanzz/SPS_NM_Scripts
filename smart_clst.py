...
def psu_clear_status(ipmi, psu_addr, psu_status_register):
     ...
     rsp = send_raw_pmbus_write_cmd_extend_py(ipmi, d9h_trans_type_block_write , d9h_pec_report, d9h_pec_en, bus , target_addr , 0 , 0 , 0 , d9h_trans_potocol_pmbus , write_len , read_len , pmbus_cmd_aray)
     # Check pmbus rsp  data
     if(rsp == ERROR):
         print('ERROR!!! send pmbus error ! respond data error')
         return ERROR
...
def psu_status(ipmi, psu_addr):
     ...
     for register in PMBUS_STATUS_REGISTERS:
     	...
     	if(register == PMBUS_STATUS_WORD_79H):
     		...
     		read_len = 2
     	rsp = send_raw_pmbus_write_cmd_extend_py(ipmi, d9h_trans_type_block_wr_pro_call , d9h_pec_report, d9h_pec_en, bus , target_addr , 0 , 0 , 0 , d9h_trans_potocol_pmbus , write_len , read_len , pmbus_cmd_aray)
     	# Check pmbus rsp  data
     	if(rsp == ERROR):
         	print('ERROR!!! send pmbus error ! respond data error')
         	return ERROR
